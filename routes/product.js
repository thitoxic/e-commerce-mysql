...
router.get("/list", async (req, res) => {
  try {
    const { searchKeyword, sortBy, order } = req.query;
    ...
    if (searchKeyword) {
      search = {
        [Op.or]: [{ productName: { [Op.like]: `%${searchKeyword}%` } }],
      };
    }
...
router.put("/update/:productId", async (req, res) => {
  try {
    ...
    for (const key in updateProps) {
      if (
        updateProps.hasOwnProperty(key) &&
        productToUpdate[key] !== updateProps[key]
      ) {
